...
test=ds_some_rows_v1 %>%
  #use mutate(new column = recode(current column to be recoded, "variable in column"="new variable name"))
    mutate(test = recode(learn_about_study_int, "1"="Flyer","3"="Facebook","5"="Other"))
...
total_pass <- ds_some_rows_v1%>%
  select(int_phone_pass_fail,learn_about_study_int,encounter_date_int)%>%
  separate(encounter_date_int,c("y","m","d"))%>%
  filter(int_phone_pass_fail==1)%>%
  group_by(m,y,int_phone_pass_fail,learn_about_study_int)%>%
  summarize(count=n())%>%
  unite("YM",c("y","m"),sep ="-")
...
t + geom_histogram(aes(fill=factor(learn_about_study_int)), stat = "Identity",
                   bins=24,
                   col="black", 
                   size=.1) + # change number of bins
  # geom_density() + #this function is meant to draw a trend line for the graph
...
social_pass <- ds_some_rows_v1%>%
  select(int_phone_pass_fail,learn_about_study_int,encounter_date_int)%>%
  separate(encounter_date_int,c("y","m","d"))%>%
  filter(learn_about_study_int==3, int_phone_pass_fail==1)%>%
...
flyer_pass <- ds_some_rows_v1%>%
  select(int_phone_pass_fail,learn_about_study_int,encounter_date_int)%>%
  separate(encounter_date_int,c("y","m","d"))%>%
  filter(learn_about_study_int==1, int_phone_pass_fail==1)%>%
  group_by(m,y,int_phone_pass_fail)%>%
  summarize(count=n())%>%
  unite("YM",c("y","m"),sep ="-")
...
Mat_age <- ds_some_rows_v1%>%
  select(record_id,analysis_mat_age,learn_about_study_int)%>%
...
Mat_age$learn_about_study_int <- as.factor(Mat_age$learn_about_study_int)

    library(ggplot2)
  theme_set(theme_classic())
  
  # Plot
  g <- ggplot(Mat_age, aes(learn_about_study_int, analysis_mat_age))
...
Education <- ds_some_rows_v1%>%
  select(record_id,mom3t_education_2)%>%
  filter(!is.na(mom3t_education_2))%>%
  group_by(mom3t_education_2)%>%
  summarise(count=n())

g <- ggplot(Education, aes(mom3t_education_2, count))
g + geom_bar(stat="identity", width = 0.5, fill="tomato2") + 
...
Education_encounter <- ds_some_rows_v1%>%
  select(record_id,mom3t_education_2,learn_about_study_int)%>%
  filter(!is.na(mom3t_education_2))%>%
  filter(!is.na(learn_about_study_int))%>%
  group_by(mom3t_education_2,learn_about_study_int)%>%
  summarise(count=n())
...
# distribution of visits each month(histogram)
h <- ggplot(Education_encounter, aes(learn_about_study_int, count)) + scale_fill_brewer(palette = "Spectral")
h + geom_histogram(aes(fill=factor(mom3t_education_2)), stat = "Identity",
                   bins=24,
                   col="black", 
                   size=.1) + # change number of bins
  # geom_density() + #this function is meant to draw a trend line for the graph
...
```{r, echo=FALSE, fig.cap="fig4.Birth/pregnant"}
A_Preg <- ds_some_rows_v1%>%
  select(record_id,learn_about_study_int)%>%
  slice(57:84)%>%
  group_by(learn_about_study_int)%>%
  summarise(count=n())
print(A_Preg)

g <- ggplot(A_Preg, aes(learn_about_study_int, count))
g + geom_bar(stat="identity", width = 0.5, fill="tomato2") + 
...
g <- ggplot(C_Preg, aes(learn_about_study_int, count))
g + geom_bar(stat="identity", width = 0.5, fill="tomato2") + 
