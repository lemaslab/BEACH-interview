---
title: "Yearly recuritment and concent analysis"
author: "Luran Manfio"
date: "12/11/2019"
output:
  html_document:
...
```
## Number of People Who Contacted Through Facebook per Year
```{r echo=FALSE, warning=FALSE}

Yearly_recruitment <- ds_some_rows_v1%>%
  select(learn_about_study,encounter_date_int)%>%
  separate(encounter_date_int,c("y","m","d"))%>%
  filter(learn_about_study=="Facebook")%>%
  group_by(y,learn_about_study)%>%
  summarise(count=n())%>%
  rename("Encounter per Year"=count,"Year of Encounter"=y,"Method of Encounter"=learn_about_study)

formattable(Yearly_recruitment, align =c("l","c","r"))
```

## Number of Facebook Recruitment per Year
```{r echo=FALSE, warning=FALSE}

Particpants_Recruited_Facebook <- ds_some_rows_v1%>%
   select(learn_about_study,encounter_date_int)%>%
  slice(57:103)%>%
  separate(encounter_date_int,c("y","m","d"))%>%
  filter(learn_about_study=="Facebook")%>%
  group_by(y,learn_about_study)%>%
  summarise(count=n())%>%
  rename("Recruitment per Year"=count,"Year of Recruitment"=y,"Method of Encounter"=learn_about_study)

formattable(Particpants_Recruited_Facebook, align =c("l","c","r"))
