...
### One of the major reasons for research delay has been recuritment isues. 9 out of 10 trials require the origional timeline to be extended due to lack of particiapnts. The problem with recruitment stems from a lack of inovention with the field and use of out of date practices. However, new methods have arisen that can prove to be a solution to the issues with current recruitment methods. The use of Facebook for recuritment solves the problems of particiapnt targeting and recruitment rates. Within the BEACH Interview study we recruited both through flyers and Facebook; however, to trully test the effectivness of the Facebook method againest the tradtional method of flyering we only conducted an ad for discrete periods of time. Following the study's completion we analysed our recruitment and found statistically signifigant differences between our two groups: pregnant and breastfeeding moms.

### The Objectives of this study are to:
### 1.) Preform exploritory data analysis to discover statistically signifigant difference between tradtional and modern recuritment methods.
### 2.) Confirm analysison the BEACH study to prove support the analysis found in this study. 
### 3.) Constuct a reactive shinyR server that will allow for more responsive and accuracte recuritment. 

### Below we have the numbers of particiaptns we recruited per quarter broken down by how they heard of our study. We show both screened and consented participants over the course of the study. 
...
### We focused our recruitment around 2 groups: Breasfeeding and Pregnant mother with BMI's that would represent both normal weight and overweight mothers. For this reason we hoped to recruit a wide rage of mother with varying BMI and we hoped that Facebook would provide mothers of a wide rage of BMI and age. We tested the varance between recruitment methods and provided a visualization of both BMI and age in boxplots. Below is a review of that research over the whole study without specifying pergant or breastfeeding.
...
### Below is a tabulation of the mean, median, and varance between Facebook and flyers to test for any statistical signifigance. blow represents mean, median, and an F-test to test for vareance. 
...
### Below we have the statstics of age.
```{r echo=FALSE, warning=FALSE}
BMI <- ds_some_rows_v1%>%
...
### Below we analysed our pregnant group first against their BMI's and ages to test for any statsitical signifigance between recuritment methods. 
...
### We preformed statistical analysis within the group against recurtment methods and tested mean, median, and variance. 
```{r echo=FALSE, warning=FALSE}
BMI <- ds_some_rows_v1%>%
...
### Below we analysed our Breastfeeding group against their BMI's and ages to test for any statsitical signifigance between recuritment methods. 
